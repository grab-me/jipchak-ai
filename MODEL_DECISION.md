...
### 검증 부족
- ~~**n=1**: GraspNet `example_data` 1장만 검증.~~ → **해소**: Cornell dataset 다운 후 정량 평가 86/89 = **0.97 IoU** 재현 (학습 시 발표 수치 일치, 모델 정상 동작 확정) + 무작위 5장 정성 시각화 일관성 우수
- **인형 도메인 미검증**: 봉제 인형 RGB-D 0장 테스트. 분포 갭 가능성 — RealSense 셋업 후 별도 검증
- **다양한 scene 미검증**: cluttered 환경(여러 물체+박스 벽+그림자) 일반화 정확도 미측정. Cornell 은 단일 사물 / 흰 배경 클로즈업이라 한계
...
### Antipodal hot spot ≠ 무게중심 (인형뽑기 핵심 위험)
- 모델은 "**잡기 쉬워 보이는 위치**" (좁은 평행면, 손가락 안착 가능 영역) 를 학습한 것이지 "**물체의 무게중심 / 들어올릴 수 있는 위치**"를 학습한 게 아님.
- 학습 레이블은 사람이 "여기 양쪽에 두면 잡힐 것 같다"고 그린 박스 — 무게/물리 정보 0.
- 인형뽑기 함의: 봉제 인형의 머리(좁고 둥글) 가 hot spot 으로 추천될 수 있는데, 머리만 잡으면 들다 떨어짐 (실제 무게중심은 몸통).
- **단기 완화**: depth 정보로 두꺼운 영역 가산점 / workspace mask
- **중기 해결**: 인형뽑기 시연(실제 잡기 성공/실패) 데이터로 fine-tune
- 사용자(이채목)의 직관적 의문 — "무게중심을 잡나?" — 이 위험을 정확히 짚음. fine-tune 필요성의 가장 강한 근거.
