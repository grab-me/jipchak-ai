...
[4] 도메인 갭 검증        ⏳ 진행 중 — 인형뽑기 환경 데이터 필요
[5] Fine-tune (도메인)    ⏳ RealSense 셋업 후
[6] 시스템 통합           ⏳ 메인 레포에서
```
...
1. RealSense D405 셋업 (하드웨어, **다음 unblocker**)
2. 인형뽑기 박스 + 봉제 인형 RGB-D 데이터 수집 (~500~2000장)
3. `train_network.py` 에 `--pretrained` 옵션 patch (10분, FINETUNE_PLAN.md 참고)
4. fine-tune 실행 → 도메인 적응 모델
5. 메인 레포(jipchak)와 통합
